...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Editar Incidente #{{ $incident->id }}</h1>
            ...
            <form action="{{ route('incidents.update', $incident) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="trip_id" id="trip_id" required
                            ...
                            @foreach($trips as $trip)
                                <option value="{{ $trip->id }}" {{ (old('trip_id') ?? $incident->trip_id) == $trip->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="mecanico" {{ (old('type') ?? $incident->type) == 'mecanico' ? 'selected' : '' }}>Mecánico</option>
                            <option value="accidente" {{ (old('type') ?? $incident->type) == 'accidente' ? 'selected' : '' }}>Accidente</option>
                            <option value="retraso" {{ (old('type') ?? $incident->type) == 'retraso' ? 'selected' : '' }}>Retraso</option>
                    ...
                    <div>
                        ...
                        <select name="severity" id="severity" required
                            ...
                            <option value="baja" {{ (old('severity') ?? $incident->severity) == 'baja' ? 'selected' : '' }}>Baja</option>
                            <option value="media" {{ (old('severity') ?? $incident->severity) == 'media' ? 'selected' : '' }}>Media</option>
                            <option value="alta" {{ (old('severity') ?? $incident->severity) == 'alta' ? 'selected' : '' }}>Alta</option>
                            <option value="critica" {{ (old('severity') ?? $incident->severity) == 'critica' ? 'selected' : '' }}>Crítica</option>
                        </select>
                        @error('severity')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="reported_at" id="reported_at" required 
                            value="{{ old('reported_at') ?? $incident->reported_at->format('Y-m-d\TH:i') }}"
                            class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm">
                        @error('reported_at')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="resolution_status" id="resolution_status" required
                            class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm">
                            <option value="pendiente" {{ (old('resolution_status') ?? $incident->resolution_status) == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                            <option value="en_proceso" {{ (old('resolution_status') ?? $incident->resolution_status) == 'en_proceso' ? 'selected' : '' }}>En Proceso</option>
                            <option value="resuelto" {{ (old('resolution_status') ?? $incident->resolution_status) == 'resuelto' ? 'selected' : '' }}>Resuelto</option>
                        </select>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm"
                        placeholder="Describe el incidente">{{ old('description') ?? $incident->description }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="actions_taken" id="actions_taken" rows="4"
                        class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm"
                        placeholder="Describe las acciones tomadas">{{ old('actions_taken') ?? $incident->actions_taken }}</textarea>
                    @error('actions_taken')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
